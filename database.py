...
def insert_todo(todo: Todo):
    ...
    with conn:
        c.execute('INSERT INTO todos VALUES (:deploy, :version, :date_added, :date_completed, :status, :position)',
        {'deploy': todo.deploy, 'version': todo.version, 'date_added': todo.date_added,
...
def update_todo(position: int, deploy: str, version: str):
    with conn:
        if deploy is not None and version is not None:
            c.execute('UPDATE todos SET deploy = :deploy, version = :version WHERE position = :position',
                      {'position': position, 'deploy': deploy, 'version': version})
